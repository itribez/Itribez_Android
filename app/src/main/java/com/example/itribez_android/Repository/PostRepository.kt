...
*/
    suspend fun likePost(postId: String, likeRequest: LikeRequest): Response<LikeResponse>? {
        return UserApi.getApi()?.likePost(bearerToken, postId, likeRequest)
    }

    suspend fun unlikePost(postId: String, unlikeRequest: UnlikeRequest): Response<UnlikeResponse>? {
        return UserApi.getApi()?.unlikePost(bearerToken, postId, unlikeRequest)
    }
    ...
    suspend fun getComments(postId: String): Response<CommentResponse>? {
        var response: Response<CommentResponse>? = null
        Log.d("PostRepository", "Bearer Token: $bearerToken")
        return try {
            Log.d("PostRepository", "Bearer Token: $bearerToken")
            val response = UserApi.getApi()?.getComments("$bearerToken",postId)
